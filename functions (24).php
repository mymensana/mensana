...
// =================================================================
// CRITICAL FIX: Add missing schedule data function
// =================================================================
if (!function_exists('thriveed_get_custom_course_schedule_data')) {
    function thriveed_get_custom_course_schedule_data($course_id) {
        $utc_datetime = get_post_meta($course_id, '_thriveed_utc_datetime', true);
        ...
        try {
            ...
            $datetime_obj = new DateTime($utc_datetime, $utc_timezone);
            
            // Convert to site timezone for display
            $site_timezone = wp_timezone();
            $local_datetime = clone $datetime_obj;
            $local_datetime->setTimezone($site_timezone);
            
            return [
                'utc_datetime' => $utc_datetime,
                'local_datetime' => $local_datetime->format('Y-m-d H:i:s'),
                'full_string' => $local_datetime->format('F j, Y \a\t g:i A T'),
                'display' => $local_datetime->format('M j, Y - g:i A')
            ];
        } catch (Exception $e) {
            error_log('ThriveEd Schedule Error: ' . $e->getMessage());
            return null;
        }
    }
}