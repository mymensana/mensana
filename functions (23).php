...
/**
 // Add this to fix the instructor assignment issue
function thriveed_fix_instructor_course_assignment() {
    $course_id = 2596; // Your course ID
    
    // Get the instructor user
    $instructor = get_user_by('login', 'tutor_instructor');
    if (!$instructor) {
        error_log('ThriveEd: Instructor user not found');
        return;
    }
    
    // Properly assign instructor to course
    update_post_meta($course_id, '_tutor_course_author', $instructor->ID);
    ...
    // Add to instructor's course list
    $instructor_courses = get_user_meta($instructor->ID, '_tutor_instructor_course_id', true);
    ...
    if (!in_array($course_id, $instructor_courses)) {
        ...
        update_user_meta($instructor->ID, '_tutor_instructor_course_id', $instructor_courses);
    }
    
    error_log('ThriveEd: Fixed instructor assignment for course ' . $course_id);
}

// Run this once to fix the assignment
add_action('init', 'thriveed_fix_instructor_course_assignment');
